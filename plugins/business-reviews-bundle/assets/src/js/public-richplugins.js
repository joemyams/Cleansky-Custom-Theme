    ...
    Tag: function(rootEl) {
        ...
        return THIS = {
            ...
            init: function() {
                ...
                if (starsEl) {
                    THIS.starsInit(starsEl);
                }

                if (starsReviewUsEl) {
                    THIS.starsInit(starsReviewUsEl);
        ...
        return THIS = {
            ...
            stylesInit: function() {
                ...
                if (options.slider_review_height) {
                    style += 'r-p [data-rs] rp-review rp-review-text' +
            ...
            reviewsInit: function() {
                let reviewsEl = rootEl.querySelectorAll('rp-review');
                for (let i = 0; i < reviewsEl.length; i++) {
                    THIS.reviewInit(reviewsEl[i]);
                }
            },

            reviewInit: function(reviewEl) {
            ...
            */
            ...
            loadAjaxReviews: function(list, offset, size) {
                ...
                RichPlugins.Utils.ajax(url, function(json) {
                    ...
                    for (var i = 0; i < len; i++) {
                        let el = list.shift();
                        THIS.reviewInit(THIS.convertReviewEl(el, json.reviews[i]));
                    }
                    while(list.length) {
                        let el = list.shift();
            ...
            convertReviewEl: function(el, review) {
                let img   = el.querySelector('rp-review-img img'),
            ...
            dotsInit: function() {
                ...
                var dotsCount = Math.round(reviewsCount / THIS.swipePerDot());

                dotsEl.innerHTML = '';
                for (var i = 1; i <= dotsCount; i++) {
                    var dot = document.createElement('rp-dot');
            ...
            swipeAuto: function() {
                if (THIS.isScrollEnd()) {
                    THIS.scroll(-reviewsCount);
                } else {
                    THIS.scroll(THIS.swipeStep());
                }
                THIS.swipeAutoStart();
            },
