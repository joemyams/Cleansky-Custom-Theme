...
class View2 {
    ...
    public function render_tag($coll_id, $bizs, $revs, $opts) {
        ...
        switch ($opts->tag_click) {
            case 'reviews':
                $tag_click = ' onclick="window.open(\'' . $this->get_allreview_url($bizs[1]) . '\', \'_blank\')"';
                break;
            default:
                $tag_click = '';
        }
        ...
        <rp-tag data-id="<?php echo $coll_id; ?>" data-opts='<?php echo $this->tag_options($opts); ?>' class="<?php echo implode(' ', $tag_cls); ?> "data-exec="">
            ...
            <rp-tag-inner class="<?php echo implode(' ', $inner_cls); ?>"<?php echo $tag_click; ?>>
                ...
                <rp-stars-wrap>
                    ...
                    <rp-stars data-info="<?php echo $stars_info2; ?>" data-reviewus="<?php echo $this->get_writereview_url($bizs[1]); ?>"></rp-stars>
                    <?php } ?>
                </rp-stars-wrap>
    ...
    public function render_slider($coll_id, $bizs, $revs, $opts) {
        ...
        <rp-slider data-id="<?php echo $coll_id; ?>" data-count="<?php echo $count; ?>" data-offset="<?php echo $offset; ?>" data-rs="<?php echo $opts->review_style; ?>" data-opts='<?php echo $this->slider_options($opts); ?>' data-exec="" <?php if ($opts->dark_theme) { ?>data-color="dark"<?php } ?>>
            <?php if (!$opts->slider_hide_head && count($bizs) > 0) { ?>
            <rp-header>
                <?php if (!$opts->header_hide_name) { ?>
                <rp-scale><?php echo __($this->scale($bizs[0]->rating), 'brb'); ?></rp-scale>
                <?php } ?>
                <rp-score>
                    <rp-rating><?php echo $bizs[0]->rating; ?></rp-rating>
                    <rp-stars data-info="<?php echo implode(',', array($bizs[0]->rating, '', $opts->color_stars)); ?>">
                        <?php echo $this->stars2($bizs[0]->rating); ?>
                    </rp-stars>
                </rp-score>
                <?php if (!$opts->header_hide_count) { ?>
                <rp-based>
                    <?php printf(esc_html__('Based on %s reviews from', 'brb'), $bizs[0]->review_count); $this->logo($bizs[0]->platform); ?>
                </rp-based>
                <?php }
                if (!$opts->header_hide_write) {
                    $this->review_us_on($bizs[0]);
                }
                ?>
            </rp-header>
            <?php } if ($count > 0) { ?>
    ...
    private function review($review, $opts) {
        ...
        <rp-review>
            <rp-review-inner>
                ...
                <rp-review-text><?php echo $review->text; ?></rp-review-text>
    ...
    private function review_head($review, $opts) {
        ...
        <rp-review-head>
            ...
            <rp-review-img>
                <img src="<?php echo $review->author_avatar?>" <?php if ($opts->lazy_load_img) { ?>loading="lazy"<?php } ?> alt="<?php echo isset($review->author_name) ? $review->author_name : 'Review author'; ?>">
            </rp-review-img>
            <?php } ?>
            <rp-review-info><?php
    ...
    private function review_us_on($biz) {
        ...
        ?>
        <rp-review_us onclick="_rplg_popup('<?php echo $this->get_writereview_url($biz); ?>', 800, 600)">
