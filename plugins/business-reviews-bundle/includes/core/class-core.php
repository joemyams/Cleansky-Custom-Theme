...
class Core {
    ...
    public function get_data($connection) {
        ...
        if (($options->summary_rating || in_array($options->view_mode, BRB_NEW_LAYOUTS)) && count($bizs) > 0) {
            ...
            $summary_wr    = isset($options->summary_wr)    && strlen($options->summary_wr)    > 0 ? $options->summary_wr    : $first_biz->id;
            array_unshift($businesses, $this->merge_biz($bizs, 'summary', $summary_name, $summary_url, $summary_photo, $summary_wr, 'summary'));
        }
    ...
    public function get_google_reviews_all($google_biz, $options) {
        ...
        $api_url = "https://app.richplugins.com/gmb/" . $google_biz->id . "/reviews?auth_code=" . $auth_code;
        ...
        if (isset($google_response_json->reviews)) {
            ...
            foreach ($google_response_json->reviews as $rev) {
                ...
                $author_url = $options->disable_google_link ? '' : 'https://search.google.com/local/reviews?placeid=' . $google_biz->props->place_id;
    ...
    public function get_facebook_reviews($facebook_biz, $options) {
        ...
        $access_token = isset($facebook_biz->props) && isset($facebook_biz->props->access_token) ?
                        $facebook_biz->props->access_token :
                        $facebook_biz->access_token;
    ...
    private function merge_biz($businesses, $id = '', $name = '', $url = '', $photo = '', $wr = '', $provider = '') {
        $count = 0;
        $rating = 0;
        $review_count = array();
        ...
        foreach ($businesses as $business) {
            ...
            if ($business->id == $wr) {
                $biz_merge->wr = $business->provider . ':' . $wr;
            }
        }
